...
void BoxColliderComponent::Uninit()
{
	transform->Uninit();
	delete transform;
}
...
void BoxColliderComponent::MoveCollision()
{
...
std::tuple<bool, GameObject*, std::list<GameObject*>> BoxColliderComponent::GetCollision()
{
	...
	for (int i = 0; i < 2; i++) {
		...
		for (auto obj : gameObjectList)
		{
			...
			if (obj->GetComponent<Collider>()->GetColliderKind() == BOX)
			{
				...
				rotation = obj->GetComponent<Collider>()->GetRotation();

				Box otherBox = obj->GetComponent<BoxColliderComponent>()->GetOBB();
			...
			else if (obj->GetComponent<Collider>()->GetColliderKind() == CAPSULE)
			{
...
//bool BoxColliderComponent::BoxsCollision(Box obb1, Box obb2)
//{
//	
