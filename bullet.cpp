...
void Bullet::Init()
{
	...
	AddComponent<BoxColliderComponent>()->SetTransform(
	...
	GetComponent<Collider>()->SetSize(XMFLOAT3(1.0f, 1.0f, 1.0f));
	Tag = "Bullet";
...
void Bullet::Update()
{
	...
	GetComponent<BoxColliderComponent>()->SetOldPosition(pos);
	float x = sinf(rot.y);
	float z = cosf(rot.y);
	...
	//当たり判定を取得
	if (std::get<0>(GetComponent<BoxColliderComponent>()->GetCollision()) == true) {
		//当たっている全てのオブジェクトのリストを取得
		std::list<GameObject*> objs = std::get<2>(GetComponent<Collider>()->GetCollision());
