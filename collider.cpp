...
void Collider::RotateAroundInit(XMFLOAT3 targetPos)
{
...
void Collider::RotateAround(float rotateX, float rotateY)
{
...
XMFLOAT3 Collider::GetRotatePosition(XMFLOAT3 targetPos) const
{
...
XMFLOAT3 Collider::clampToBox(const XMFLOAT3 point, const XMFLOAT3 halfSize)
{
...
bool Collider::BoxsCollision(Box obb1, Box obb2)
{
...
float Collider::SegmentDistance(const XMFLOAT3 vec1start, const XMFLOAT3 vec1end, 
	                            const XMFLOAT3 vec2start, const XMFLOAT3 vec2end, float& t1, float& t2)
{
...
bool Collider::CapsulesCollision(GameObject* capsuleCollider1, GameObject* capsuleCollider2)
{
	...
	if (distanceSquared <= radiusSum * radiusSum)
	{
		...
		if (capsuleCollider2->GetObjectKind() == Static)
		{
			...
			CapsuleColliderComponent* col = capsuleCollider1->GetComponent<CapsuleColliderComponent>();
		...
		else if (capsuleCollider2->GetObjectKind() == Dynamic)
		{
			...
			if (weight2 > weight1)
			{
				...
				CapsuleColliderComponent* col1 = capsuleCollider1->GetComponent<CapsuleColliderComponent>();
				Transform* trans1 = capsuleCollider1->GetComponent<Transform>();
			...
			{
				//相手を動かす
				CapsuleColliderComponent* col2 = capsuleCollider2->GetComponent<CapsuleColliderComponent>();
				Transform* trans2 = capsuleCollider2->GetComponent<Transform>();
...
bool Collider::CapsuleBoxCollision(GameObject* capsuleObj, GameObject* boxObj)
{
	const float MIN = 0.00001f; //無視する値

	const int MAX_COUNT = 10;//最大処理回数

	Box box = boxObj->GetComponent<BoxColliderComponent>()->GetOBB();

	Angle angle;

	Capsule capsule = capsuleObj->GetComponent<CapsuleColliderComponent>()->GetCapsule();
		...
		else
		{
			...
			else if (boxObj->GetObjectKind() == Dynamic)
			{
				...
				if (boxObj->GetWeight() > m_GameObject->GetWeight())
				{
					capsule.startPos = Add(capsule.startPos, reverseVector);
					capsule.endPos = Add(capsule.endPos, reverseVector);
					XMFLOAT3 Pos = boxObj->GetComponent<Collider>()->GetPosition();
					Pos = Add(Pos, reverseVector);

					boxObj->GetComponent<Collider>()->SetPosition(Pos);
