...
void PlayerCamera::CameraControl()
{
	...
	if (GetAsyncKeyState(VK_RBUTTON))
	{
		...
		GetComponent<Circle2DCollider>()->CircleDrawState(true);
		GetComponent<Circle2DCollider>()->CircleUpdateState(true);
	...
	else
	{
		...
		GetComponent<Circle2DCollider>()->CircleDrawState(false);
		GetComponent<Circle2DCollider>()->CircleUpdateState(false);
		float len = GetComponent<Transform>()->GetLength();
		if (len < m_Len)
		{
			GetComponent<Transform>()->SetLength(len + 0.05f);
...
bool PlayerCamera::CheckView(XMFLOAT3)
{
