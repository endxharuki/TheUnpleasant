...
void Star::Init()
{
	...
	AddComponent<SphereColliderComponent>()->SetSize(XMFLOAT3(1.0f, 1.0f, 1.0f));
	GetComponent<SphereColliderComponent>()->SetPosition(XMFLOAT3(0.0f, 10.0f, 0.0f));
...
void Star::Update()
{
	...
	GetComponent<Collider>()->SetPosition(pos);
	...
	//if (GetComponent<Collider>()->GetColliderTag() == "Sphere")
	//{
	//	std::tuple<bool, GameObject*, std::list<GameObject*>> list = GetComponent<SphereColliderComponent>()->GetCollision();
	...
	//		{
	//			if (onCollisionObj->GetComponent<Collider>()->GetColliderTag() == "Box")
	//			{
